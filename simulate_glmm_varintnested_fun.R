sim.glmm.varintnested <- function(
  ...
  # Create group labels.
  groups0 <- data.frame(group0 = as.factor(sort(rep(c(1:J0), I * J1))))
  if (nested) {
    groups1 <- as.factor( paste0( sort(rep(c(1:J0), I * J1)) , "_" , rep(sort(rep(c(1:J1), I)), J0)))
  } else {
    groups1 <- as.factor( rep(c(1:J1), I))
  }
